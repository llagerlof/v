...
- markdown `**bold**` rendered bold, with faint asterisks
...
src/
  ...
  emphasis.rs   markdown `**bold**` styling over plain or highlighted text
...
7. Strong emphasis styling in `emphasis.rs` (markdown extensions only).
...
- Markdown `**bold**` spans are printed bold with faint (`\x1b[2m`) asterisks; markdown extensions only, and not configurable. Styling runs last, on plain or highlighted text, so it adds only zero-width escapes to already wrapped lines. Spans may cross wrapped lines but not blank lines; fenced code, indented code, inline code spans and `\*` escapes are skipped.
